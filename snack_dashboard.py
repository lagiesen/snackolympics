...
for label in RATING_LABELS:
    st.markdown(f"<div class='section-label'>The top snacks in the category {label.upper()} are:</div>", unsafe_allow_html=True)
    ...
    for i, row in top3.iterrows():
        ...
        <div class="snack-card">
            ...
            <div class="stars">{stars(row[label])}</div>
    ...
    try:
        ...
        if len(shared_snacks) > 0:
            ...
            for cat in PREFERRED_CATS:
                if len(found_cats) == 2:
                    break
                # Average rating both gave this snack in this category
                best_snack = None
                best_avg = -1
                for snack in shared_snacks:
                    avg = (p1_data.loc[snack, cat] + p2_data.loc[snack, cat]) / 2
                    if avg > best_avg:
                        best_avg = avg
                        best_snack = snack
                if best_snack and best_avg >= 4:
                    found_cats.append((cat, best_snack, best_avg))

            # If we couldn't fill 2 slots with score >= 4, relax threshold
            if len(found_cats) < 2:
                found_cats = []
                for cat in PREFERRED_CATS:
                    if len(found_cats) == 2:
            ...
            for cat, snack, avg in found_cats:
                agreement_sentences.append(
                    f"You both really liked the <b>{cat.lower()}</b> of <b>{snack}</b> ⭐ {stars(avg)}"
                )
    except KeyError:
        agreement_sentences = []
